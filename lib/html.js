...
function renderMovieCard(movie, timeZone) {
  const poster = movie.wikipedia?.posterUrl
    ? `
      <a class="movie-poster-link" href="${escapeHtml(movie.wikipedia.url)}" target="_blank" rel="noreferrer">
        <img class="movie-poster" src="${escapeHtml(movie.wikipedia.posterUrl)}" alt="Poster zu ${escapeHtml(movie.title)}" loading="lazy" referrerpolicy="no-referrer">
      </a>
    `
    : '';
...
export function renderMoviesPage({
  ...
  const warning = results?.stats.failedChannels
    ...
    : '';
  const selectedCount = settings.selectedChannels.length;
  const selectorLabel = selectedCount
    ? `${selectedCount} Sender ausgewählt`
    : 'Sender auswählen';
  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      details.channel-picker summary::-webkit-details-marker {
        display: none;
      }
      details.channel-picker summary::after {
        content: "+";
        font-size: 1.15rem;
        color: var(--accent);
      }
      details.channel-picker[open] summary::after {
        content: "−";
      }
      .picker-body {
        padding: 0 10px 10px;
      ...
      .movie-poster {
        ...
        border: 1px solid var(--line);
        background: #fff;
        box-shadow: 0 8px 18px rgba(33, 24, 18, 0.12);
      }
      .movie-card::after {
        content: "";
        display: block;
      ...
      <section class="layout">
        <form class="panel" method="post" action="${escapeHtml(action)}">
          ...
          <div class="form-row">
            ...
            <details class="channel-picker"${selectedCount ? ' open' : ''}>
              <summary>${escapeHtml(selectorLabel)}</summary>
              <div class="picker-body">
                <div class="channel-grid">
                  ${channels.map((channel) => renderChannelOption(channel, selectedChannels)).join('\n')}
                </div>
                <p class="picker-hint">Mehrfachauswahl per Checkbox. Die Auswahl bleibt gespeichert.</p>
              </div>
            </details>
          </div>
